...
import kr.borntorun.api.adapter.repository.entity.QFeedEntity;
...
public class FeedQuery {
  ...
  public Page<FeedEntity> searchAllByFilter(final SearchAllFeedQuery query, final Pageable pageable) {
    ...
    BooleanExpression whereClause = feed.isDeleted.isFalse();
    ...
    // 내 크루만 공개
    if(query.isMyCrew()) {
      whereClause = whereClause.and(feed.userEntity.crewId.eq(query.my().getCrewId())
    ...
    if(query.isUsedIntegratedSearch()) {
      final BooleanExpression searchWhereClause = feed.contents.matches(query.searchKeyword());

      whereClause = !query.searchedUserIds().isEmpty() ?
          whereClause.and(searchWhereClause.or(feed.userId.in(query.searchedUserIds()))) :
